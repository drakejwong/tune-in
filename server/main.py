...
from getUserTops import getTopTracks, getTopArtists
...
term = "short_term"
tracks_list = getTopTracks(term)
artists_list = getTopArtists(term)
...
# with session_scope(db) as session:
    # db.create_party(1263126600, session)
    # db.delete_party_data('markhor-of-inevitable-sympathy', PartyTracks, session)
...
def update_user_data(db, session):
    ...
    # dummy_id = '696969'
    # db.delete_user_data(dummy_id, TopTracks, session)
    # db.delete_user_data(dummy_id, TopArtists, session)
    # db.delete_user_data(dummy_id, Users, session)
    # if db.user_exists_in_table(dummy_id, Users, session):
    #     db.updateLoginTime(dummy_id, session)
    #     db.updateUserTops(dummy_id, tracks_list, artists_list, session)
    # else:
    #     db.createUser(dummy_id, user_name, user_country, user_profile_pic, session)
    #     db.saveUserTops(dummy_id, tracks_list, artists_list, session)

def preview_party_playlist(): 
    # on click, calculates seeds for users in party and displays recommended tracks
    ...
    with session_scope(db) as session:
        ...
        results = recommendTracks(tracks=seed_tracks, artists=seed_artists)
    ...
    # display proposed tracks on front-end
    for track in results['tracks']:
        print("recommendation:", track['name'], "by", track['artists'][0]['name'])
...
# generate("Your Party's Top Hits", recs, party_user_list)
# (self, parameter_list)
